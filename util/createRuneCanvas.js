  ...
  createRune: async (rune, type) => {
    ...
    try {
      ...
      const rarityFrame = await Canvas.loadImage(
        `${process.env.POX_IMG_URL}/_themes/global/frames/large/lg_frame_rarity_${getFrameRarity(
          rune.rarity
        )}.gif`
      );
      ...
      const factionIconTwo = await Canvas.loadImage(
        `${process.env.POX_IMG_URL}/_themes/global/frames/large/faction_${split.length > 1 ? split[1] : split[0]}_2.png`
      )
