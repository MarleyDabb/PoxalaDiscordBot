...
module.exports = {
  displayAbilities: async (abilityArray, startingX, startingY, context) => {
    const arr = abilityArray.sort((a, b) => (a.default === b.default)? 0 : a.default? -1 : 1);
    for (let i = 0; i < arr.length; i++) {
      ...
      let abilityBg;
      if (ability.default) abilityBg = await Canvas.loadImage(path.join(__dirname, '..', '/images/abilityBg3.png'));
      else if (ability.default === undefined) abilityBg = await Canvas.loadImage(path.join(__dirname, '..', '/images/abilityBg3.png'))
      else abilityBg = await Canvas.loadImage(path.join(__dirname, '..', '/images/abilityBgSet.png'));
      ...
      context.fillText(ability.name, startingX + 50, (startingY + 10) + (i * toAddY));

      context.fillStyle = '#1d4597';
      context.textAlign = 'right';
      context.fillText(`${ability.noraCost} Nora`, startingX + 496, (startingY + 10) + (i * toAddY));

      context.fillStyle = '#3c3c3c';
      context.fillText(`AP: ${ability.apCost}`, startingX + 441, (startingY + 10) + (i * toAddY));

      context.fillStyle = '#3c3c3c';
      context.fillText(`CD: ${ability.cooldown}`, startingX + 386, (startingY + 10) + (i * toAddY));
